...
## 5. 自動更新の設定（GitHub Actions）
すでに `.github/workflows/daily_update.yml` を作成済みですので、上記のファイルをアップロード（Push）するだけで、**毎日 AM 1:00 (日本時間)** に自動的に `daily_events.json` が更新されるようになります。

1. GitHubのリポジトリページで「**Actions**」タブを開きます。
2. 左側に「Daily Data Update」というワークフローが表示されていれば設定完了です。
3. （テスト実行）もし今すぐ動かしたい場合は、「Run workflow」ボタンから手動で実行することも可能です。
